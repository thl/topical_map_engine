class BlurbsController < AclController
  def initialize
    super
    @guest_perms = []
  end
  ...
  def create
    ...
    respond_to do |format|
      if @blurb.save
        format.html { redirect_to(blurbs_url, :notice => 'Blurb was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @blurb.update_attributes(params[:blurb])
        format.html { redirect_to(blurbs_url, :notice => 'Blurb was successfully updated.') }
        format.xml  { head :ok }
      else
