...
module ApplicationHelper
  ...
  def side_column_links
    ...
    str += "<li>#{link_to 'Home', root_path, {:hreflang => 'Lists all categories.'}}</li>\n"
  	...
  	authorized_only(hash_for_complex_scripts_languages_path) { str += "<li>#{link_to 'Languages', complex_scripts_languages_path, {:hreflang => 'Manage languages used for translation of titles and interface.'}}</li>\n" }
    authorized_only(hash_for_authenticated_system_people_path) { str += "<li>#{link_to 'People', authenticated_system_people_path, {:hreflang => 'Manage people.'}}</li>\n" }
    authorized_only(hash_for_authenticated_system_roles_path) { str += "<li>#{link_to 'Roles', authenticated_system_roles_path, {:hreflang => 'Manage roles and their permissions.'}}</li>\n" }
    authorized_only(hash_for_authenticated_system_permissions_path) { str += "<li>#{link_to 'Permissions', authenticated_system_permissions_path, {:hreflang => 'Manage permissions and their descriptions.'}}</li>\n" }
